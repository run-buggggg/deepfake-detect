...
def deepfake(test_path,model_path='./output/Mesonet/best.pkl'):
	torch.backends.cudnn.benchmark=True
	test_dataset = torchvision.datasets.ImageFolder(test_path, transform=mesonet_data_transforms['val'])
	test_loader = torch.utils.data.DataLoader(test_dataset, batch_size=1, shuffle=True, drop_last=False)  # num_workers=8
	model = Meso4()
	...
	with torch.no_grad():
		for (image, labels) in test_loader:
			...
			p = softmax(outputs).cpu().numpy()[0][preds]
			return preds,p
